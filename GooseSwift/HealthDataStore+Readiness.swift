...
extension HealthDataStore {
  ...
  // Must be called on a background context (packetInputQueue) or wrapped in
  // a Task — the bridge call is synchronous.
  func runReadinessV1() {
    let db = databasePath
    let bridge = self.bridge
    let now = Date()
    // Capture the live today strain on @MainActor before dispatching to background.
    ...
    packetInputQueue.async { [weak self] in
      ...
      // Local pure helpers (avoid @MainActor static method calls on background queue).
