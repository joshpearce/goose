...
- [x] **WEAR-04**: User can view a scan list of nearby HR monitors (device name + RSSI) and initiate a scan from the app
- [x] **WEAR-05**: User can tap a device in the scan list to connect the HR monitor
- [x] **WEAR-06**: User can run an HR monitor capture session independently, without requiring an active WHOOP session
...
- [x] **RTC-01**: WHOOP 4.0 clock is automatically synced to iPhone time after BLE connection when clock drift exceeds the configured threshold (resolves upstream issue #17)

### DASH — Recovery V2 Dashboard

- [x] **DASH-01**: User can view Recovery V2 dashboard with HRV, RHR, hero score, and 7-day trend, backed by live bridge data from Rust

### L10N — pt-PT Localisation

- [x] **L10N-01**: All static UI text literals are translated to European Portuguese (pt-PT) via Xcode String Catalog (`.xcstrings`)
- [x] **L10N-02**: All dynamic status strings (`@Published` BLE state, connection state, sync state) are displayed in pt-PT at the view layer
...
| WEAR-04 | Phase 10 | Complete |
| WEAR-05 | Phase 10 | Complete |
| WEAR-06 | Phase 11 | Complete |
| RTC-01 | Phase 12 | Complete |
| DASH-01 | Phase 13 | Complete |
| L10N-01 | Phase 14 | Complete |
| L10N-02 | Phase 14 | Complete |
