...
- [x] **ARCH-02**: store.rs 140 public methods → domain stores sharing `Arc<Connection>` (`SleepStore`, `CaptureStore`, `MetricsStore`, `ActivityStore`) in `store/` subdirectory; runtime schema version validation on SQLite open
