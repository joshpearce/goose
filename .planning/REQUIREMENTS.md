...
- [ ] **ARCH-03**: 133 `.unwrap()` calls in production code (bridge.rs 71, store.rs 62) → `Result<_, GooseError>`; `#[cfg_attr(not(test), deny(clippy::unwrap_used))]` added; `catch_unwind` safety net on bridge dispatcher entry point
