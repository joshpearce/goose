...
use crate::metrics::{goose_strain_v1, resolve_effective_hrmax, StrainInput};
...
pub fn detect_exercise_sessions(
  ...
  let fallback_hrmax = profile.max_hr.unwrap_or_else(|| {
    220.0 - profile.age.unwrap_or(30) as f64
  });
  ...
  for sample in hr {
    ...
    let best = smoothed_gravity.iter()
  ...
  let active: Vec<&AlignedPair> = aligned.iter()
    .filter(|p| {
      p.bpm as f64 > rhr + HR_MARGIN_BPM && p.smoothed_mag > MOTION_THRESHOLD
    })
    .collect();
  ...
  let duration_filtered: Vec<Vec<AlignedPair>> = merged.into_iter()
    .filter(|seg| {
  ...
  for seg in duration_filtered {
    ...
    let sample_hrr_pcts: Vec<f64> = seg.iter().map(|p| {
      ((p.bpm as f64 - rhr) / hrr_range * 100.0).clamp(0.0, 100.0)
    }).collect();
    ...
    let zone_of = |hrr_pct: f64| -> u8 {
      if hrr_pct < 50.0 { 1 }
      else if hrr_pct < 60.0 { 2 }
      else if hrr_pct < 70.0 { 3 }
      else if hrr_pct < 80.0 { 4 }
      else { 5 }
    };
    ...
    if !skip_intensity_gate {
      let z2plus_count = sample_hrr_pcts.iter()
    ...
    let peak_hr = seg.iter().map(|p| p.bpm as f64).fold(f64::NEG_INFINITY, f64::max);
    ...
    let strain = strain_result.output
      .as_ref()
...
mod tests {
  ...
  fn test_alignment_within_tolerance() {
    ...
    let hr = vec![HrSample { ts: 100.0, bpm: 100 }];
    ...
    assert_eq!(aligned.len(), 1, "HR at ts=100 should match gravity at ts=103 (3s < 5s)");
    ...
    assert_eq!(aligned2.len(), 0, "HR at ts=100 should NOT match gravity at ts=106 (6s > 5s)");
  }
  ...
  fn align_for_test(hr: &[HrSample], gravity: &[GravityRow]) -> Vec<AlignedPair> {
    ...
    for sample in hr {
      ...
      if let Some((_, smoothed_mag)) = best {
        aligned.push(AlignedPair { ts: sample.ts, bpm: sample.bpm, smoothed_mag: *smoothed_mag });
      }
    }
    aligned
  }
  ...
  fn make_session(duration_min: f64, fraction_z2plus: f64) -> Vec<ExerciseSession> {
    ...
    let mut hr = Vec::new();
    for i in 0..n_z2plus {
      hr.push(HrSample { ts: i as f64, bpm: z2_bpm });
    }
    for i in 0..n_z1 {
      hr.push(HrSample { ts: (n_z2plus + i) as f64, bpm: z1_bpm });
    }
    // Sort by ts
  ...
  fn test_minimum_duration_filter() {
    ...
    assert_eq!(sessions_8min.len(), 0, "8 min session should be rejected (< 10 min)");

    // Session of 11 min kept
    let sessions_11min = make_session(11.0, 0.60);
    assert_eq!(sessions_11min.len(), 1, "11 min session should be kept (>= 10 min)");
  }
  ...
  fn test_intensity_gate_discard() {
    ...
    assert_eq!(sessions_30pct.len(), 0, "Session with 30% Z2+ should be discarded");

    // Session with 55% Z2+ time kept
    let sessions_55pct = make_session(12.0, 0.55);
    assert_eq!(sessions_55pct.len(), 1, "Session with 55% Z2+ should be kept");
  }
  ...
  fn test_no_rhr_no_sessions() {
    ...
    let sessions = detect_exercise_sessions(&hr, &gravity, &profile);
    assert_eq!(sessions.len(), 0, "No RHR available → should return empty Vec");
  }
}
